...
public class MyPageService {
    ...
    public MyPageDtos.Profile profile(UUID userId) {
        ...
        List<String> types = userUserTypeRepository.findAllByUserId(userId).stream()
                .map(mapping -> mapping.getUserType().getCode())
                .toList();
        return new MyPageDtos.Profile(
                user.getNickname(),
    ...
    public MyPageDtos.Profile updateProfile(UUID userId, MyPageDtos.UpdateProfileRequest request) {
        ...
        if (request.userTypes() != null) {
            for (String code : request.userTypes()) {
    ...
    public List<MyPageDtos.Run> runs(UUID userId) {
        activeUser(userId);
        return runningRecordRepository.findAllByUserIdOrderByStartedAtDesc(userId).stream()
                .map(this::runDto)
                .toList();
    }
    ...
    public MyPageDtos.TripResponse createTrip(UUID userId, MyPageDtos.CreateTripRequest request) {
        ...
        if (request.name() == null || request.name().isBlank() || request.startDate() == null || request.endDate() == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Trip name and date range are required.");
        }
        if (request.endDate().isBefore(request.startDate())) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Trip end date must be on or after start date.");
        }
    ...
    public MyPageDtos.NotificationSettings updateNotifications(UUID userId, MyPageDtos.NotificationSettings request) {
        ...
        setting.update(
                request.recommendedCourse(),
                request.weather(),
                request.savedCourseUpdate(),
                request.meetupInvite(),
                request.commentLike(),
                request.tierChange(),
                request.eventChallenge()
        );
        return notificationDto(setting);
    }

    private MyPageDtos.Run runDto(RunningRecord record) {
        Course course = record.getCourse();
        return new MyPageDtos.Run(
                record.getId(),
    ...
    private MyPageDtos.TripResponse tripDto(Trip trip) {
        ...
        return new MyPageDtos.TripResponse(
                trip.getId(),
                trip.getName(),
                trip.getRegion(),
                trip.getStartDate(),
                trip.getEndDate(),
                trip.getThumbnailImageUrl(),
                runs.size(),
                distance,
                visitRepository.countByRunningRecordTripId(trip.getId()),
                duration
        );
    }

    private MyPageDtos.NotificationSettings notificationDto(UserNotificationSetting setting) {
        return new MyPageDtos.NotificationSettings(
                setting.getRecommendedCourse(),
                setting.getWeather(),
                setting.getSavedCourseUpdate(),
                setting.getMeetupInvite(),
                setting.getCommentLike(),
                setting.getTierChange(),
                setting.getEventChallenge()
        );
    }
