    ...
    <main className="onboarding-page">
      ...
      <section className="onboarding-content">
        ...
        {step === 3 && <>
          ...
          <p className="terms-demo-notice">공모전 시연 및 포트폴리오 운영 기준으로 작성된 약관 안내입니다.</p>
          <button type="button" className={`agree-all ${allTerms ? 'checked' : ''}`} aria-pressed={allTerms} onClick={() => { const nextValue = !allTerms; setForm((prev) => ({ ...prev, terms: { service: nextValue, privacy: nextValue, location: nextValue, marketing: nextValue } })) }}><i aria-hidden="true">✓</i><span>전체 동의 <small>선택 항목 포함</small></span></button>
          <div className="term-list">
            {TERM_ITEMS.map((term) => {
              const isOpen = openTerm === term.key
              const panelId = `term-panel-${term.key}`
              return (
                <div className={`term-item ${isOpen ? 'open' : ''}`} key={term.key}>
                  <div className="term-row">
                    ...
                    >
                      <i className={form.terms[term.key] ? 'checked' : ''} aria-hidden="true">✓</i>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <span>{term.label}</span><b aria-hidden="true">›</b>
                    </button>
                  </div>
                  {isOpen ? (
                    <div id={panelId} className="term-detail" role="region" aria-label={`${term.label} 상세 내용`}>
